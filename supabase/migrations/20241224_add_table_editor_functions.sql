...
-- Grant execute permission only to admins
GRANT EXECUTE ON FUNCTION execute_safe_sql(TEXT) TO authenticated;

-- Create RLS policy to restrict execute_safe_sql to admins only
CREATE POLICY "Only admins can execute SQL" ON profiles
  FOR ALL USING (
    EXISTS (
